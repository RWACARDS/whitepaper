...
- Two user-supplied hero videos open pages as full-width 16:9 panels via the `.video-hero` class in `style.css` (bare `<video className="video-hero" autoPlay muted loop playsInline />`, no Frame): `videos/card.mp4` on Introduction (replaces the reserved video slot) and `videos/privacy.mp4` on Trust and Security. Both are 1920x1080 silent loops; the class reserves the 16:9 box before load, rounds corners, adds a soft shadow on light and a hairline border on dark.
